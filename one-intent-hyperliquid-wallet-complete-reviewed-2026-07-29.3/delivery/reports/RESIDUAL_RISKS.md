...
- Android/iOS は安全境界のshellです。Pixel 9aのlocal UI proofと、Team `8R3B5675ZJ`のiPhone 12 signed device build／インストール・起動・Appium/WDA画面取得・tap・上下gestureは確認済みですが、overlay、accessibility、backup、attestation、biometric lifecycle、完全な実機matrix、release-bound screen evidenceは未証明です。
